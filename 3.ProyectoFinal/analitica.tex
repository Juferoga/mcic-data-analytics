...
Alcanzó 96.50\% de exactitud y precisión perfecta en Maligno
(1.000), con un Recall de 0.9057 (Fig.~\ref{fig:svm_cm}).
...
(Fig.~\ref{fig:rl_cm}). Su CV Recall $= 0.9308 \pm 0.0247$ confirma
...
\end{equation}
donde $n_b$ es el número de instancias en la rama $b$ de la partición, $n_t$ el total, $n_c$ las instancias de la clase $c$ en el subconjunto $S$, y $H$ la entropía de Shannon en bits. Alcanza 92.31\% de exactitud y Recall Maligno de 0.8302 — el más bajo del experimento, coherente con la mayor varianza de los modelos de árbol único. Su ventaja es la interpretabilidad: genera reglas SI–ENTONCES verificables por médicos (Fig.~\ref{fig:tree}).
...
y Recall de Maligno con SVM (96.50\% y 0.9057), pero su validación
cruzada 5-fold es más estable: CV Recall $= 0.9183 \pm 0.0148$
frente a $0.8933 \pm 0.0464$ del SVM, lo que indica menor varianza
...
\begin{figure}[!t]
  ...
  cruzada (CV Recall $0.9183 \pm 0.0148$).}
  \label{fig:rf_cm}
\end{figure}
...
El valor $k=1$ resultó óptimo con un Recall de $0.8990$, aunque
...
con exactitud de 90.91\% y Recall Maligno de 0.8491, reflejo
...
positivo, con una exactitud de 93.71\%. A pesar de su modesta
sensibilidad (Recall Maligno $= 0.8491$), el Naive Bayes destaca
por su precisión casi perfecta en la clase Maligno (0.9783):
cuando predice un tumor como maligno, acierta el 97.83\% de las
veces. Su validación cruzada es estable: CV Recall
$= 0.8804 \pm 0.0130$, la menor desviación estándar de todos
...
4~falsos negativos y 1~falso positivo, alcanzando 96.50\% de
exactitud y un Recall Maligno de $0.9245$ — el segundo más alto
del experimento, superado únicamente por la Regresión Logística. Con una precisión de 0.9800 en Maligno, el modelo es
altamente conservador en sus predicciones positivas. Sin embargo,
su CV Recall $= 0.8933 \pm 0.0464$ revela la mayor variabilidad
...
\begin{figure}[!t]
  ...
  negativos alcanza el segundo mejor Recall Maligno (0.9245).}
  \label{fig:cm_xgb}
\end{figure}
...
de 94.41\% y un Recall Maligno de $0.8679$. Aunque su
precisión en Maligno es alta (0.9787), su sensibilidad
es la más baja de los métodos de boosting. Su CV Recall $= 0.8933 \pm 0.0313$ coincide en media con
...
\begin{figure}[!t]
  ...
  lo sitúan en sexto lugar del ranking por Recall Maligno (0.8679).}
  \label{fig:cm_ada}
\end{figure}
...
a 0.93--0.94, coherente con el 96.50\% obtenido sobre el
conjunto de prueba. El modelo alcanza un Recall Maligno de
0.9057 y precisión perfecta en Maligno (1.000), empatando
...
\textbf{Regresión Logística} obtuvo el mejor rendimiento global: 97.90\% de exactitud y Recall Maligno de 0.9434. De los 53 tumores malignos, clasificó incorrectamente solo 3 (falsos negativos) y no produjo ningún falso positivo (precisión Maligno = 1.000), lo que significa cero pacientes sanos diagnosticados erróneamente como malignos. Su simplicidad matemática facilita además la interpretación clínica de los coeficientes. Este resultado es consistente con trabajos recientes que sitúan a la Regresión Logística entre los clasificadores de mejor desempeño sobre el WDBC, con exactitudes del orden del 97--98\% \cite{strelcenia2023, rajpoot2024}.

\textbf{ANN} obtiene un Recall Maligno de 0.9057 y exactitud de 96.50\%, empatando en todas las métricas con SVM y Random Forest (misma matriz de confusión: 48 verdaderos positivos, cero falsos positivos). El uso de Dropout y Early Stopping previno el sobreajuste (Fig.~\ref{fig:ann_train}). Que una red con 545 parámetros no supere a los modelos lineales en este dataset tiene una explicación directa: con solo 426 instancias de entrenamiento, y un problema que los modelos lineales ya resuelven con alta precisión, la mayor capacidad de la red no aporta ventaja.

\textbf{XGBoost} logra un Recall Maligno de 0.9245, el segundo mejor del experimento. Sin embargo, su estabilidad en validación cruzada es la más baja junto con SVM ($\pm$0.0464), lo que indica sensibilidad a la partición de datos de entrenamiento.

\textbf{SVM, Random Forest y ANN} comparten exactamente las mismas métricas (96.50\% de exactitud, Recall Maligno de 0.9057, precisión Maligno = 1.000, cero falsos positivos). Que un kernel lineal, un ensemble de árboles y una red neuronal coincidan refuerza la separabilidad lineal del problema con las 15 características. Las diferencias aparecen en la estabilidad: Random Forest es el más robusto (CV std $\pm$0.0148) y SVM el de mayor varianza ($\pm$0.0464).

\textbf{Árbol de Decisión} presenta el Recall Maligno más bajo (0.8302), coherente con su mayor varianza al ser un modelo único sin ensemble. Sin embargo, sigue siendo el modelo más interpretable.

\textbf{AdaBoost} alcanza el sexto lugar con un Recall Maligno de 0.8679, produciendo 7~falsos negativos y 1~falso positivo. Su precisión en Maligno (0.9787) es alta: cuando predice un tumor como maligno, casi siempre acierta. Sin embargo, su sensibilidad es la más baja entre los métodos de boosting, por detrás de XGBoost.

\textbf{Naive Bayes} comparte Recall Maligno con KNN (0.8491) pero supera en exactitud (93.71\% vs 90.91\%) y produce solo 1~falso positivo. Aunque modesto en sensibilidad, es el clasificador más conservador del experimento: estima $P(\text{Maligno}|\mathbf{x})$ de forma cautelosa, minimizando las alarmas falsas a costa de perder algunos verdaderos malignos.
...
Logística lidera en solitario con Recall Maligno de 0.9434
— casi cuatro puntos por encima del tercer bloque (0.9057, formado
por SVM, ANN y RF) — y con una desviación moderada ($\pm$0.0247).
...
segundo mejor Recall Maligno (0.9245) pero comparte con SVM la
mayor desviación estándar ($\pm$0.0464). Esto indica que su
...
($\pm$0.0130), seguido de Random Forest ($\pm$0.0148). Ambos
...
(0.8491, empatado con Naive Bayes) con 8 falsos negativos — 8 pacientes con cáncer que
...
($\pm$0.0148 vs $\pm$0.0464). Una evaluación basada únicamente
...
brecha entre el Recall en prueba (0.8491) y el CV Recall (0.8990)
— una diferencia de 0.0499 puntos, la más amplia del experimento.
...
absoluto: la distancia entre el primero (LR, 0.9434) y el tercero
(SVM/RF/ANN, 0.9057) corresponde a 2 predicciones sobre 53 instancias
...
\begin{itemize}
  ...
  Regresión Logística — mayor Recall Maligno (0.9434) con
  precisión perfecta (1.000) y coeficientes auditables.

  \item \textbf{Robustez ante variaciones en los datos}:
  Random Forest — mejor estabilidad en CV ($\pm$0.0148) con
  ...
  XGBoost — segundo mejor Recall Maligno (0.9245); su CV std
  elevado ($\pm$0.0464) requiere atención ante variaciones en
  los datos de entrenamiento.

  \item \textbf{Interpretabilidad para personal médico}:
  Árbol de Decisión — reglas SI–ENTONCES auditables, con el
  menor Recall Maligno del conjunto (0.8302).
\end{itemize}
...
\begin{itemize}
  ...
  Regresión Logística (0.9434) tiene un intervalo de
...
Se implementó un pipeline completo de analítica de datos para la detección de cáncer de mama. A continuación se sintetizan las conclusiones por fase del pipeline.

\subsection{Preprocesamiento y selección de atributos}
La \textbf{normalización z-score} fue indispensable para los algoritmos sensibles a la escala (SVM, KNN), y ajustar el escalador solo sobre el conjunto de entrenamiento evitó el \textit{data leakage} \cite{kantardzic, garcia2015}. Conservar los valores anómalos (13\% de las instancias) preservó la capacidad de detectar tumores atípicos, los casos clínicamente más críticos. La \textbf{selección por intersección de dos métodos} (correlación con la variable objetivo y separación de medias) redujo la dimensionalidad de 30 a 15 características (50\%) sin degradar el rendimiento \cite{remeseiro2019}; el criterio de correlación fue el determinante, y los atributos descartados (\textit{texture}, \textit{smoothness}, \textit{symmetry}, \textit{fractal dimension}) coincidieron con los de menor poder discriminativo identificados en el EDA.

\subsection{Modelado y evaluación}
La \textbf{Regresión Logística} obtuvo el mayor Recall Maligno (0.9434) con precisión perfecta (cero falsos positivos), la mejor combinación para screening clínico. Que un modelo lineal supere a SVM, Random Forest y ANN —los tres igualados en 0.9057— confirma que el problema es linealmente separable con las 15 características seleccionadas. La elección del Recall Maligno como métrica principal fue decisiva: bajo accuracy, KNN parecía competitivo (90.91\%), pero su sensibilidad real (0.8491) dejaba 8 tumores malignos sin detectar, diferencia que el accuracy por sí solo ocultaba \cite{hossin2015, sokolova2009}. El \textbf{Naive Bayes Gaussiano}, de fundamento bayesiano \cite{bayesian_med}, complementa el análisis con probabilidades posteriores $P(\text{Maligno}|\mathbf{x})$ que cuantifican la incertidumbre del diagnóstico.

\subsection{Estabilidad y generalización}
La \textbf{validación cruzada 5-fold} \cite{hastie2009} distinguió modelos de alto rendimiento puntual pero inestable (XGBoost y SVM, $\pm$0.0464) de otros más robustos (Random Forest $\pm$0.0148, Naive Bayes $\pm$0.0130); la Regresión Logística combinó el mejor rendimiento con estabilidad moderada ($\pm$0.0247). La elección del modelo final no depende solo de las métricas puntuales, sino del contexto de uso —screening masivo, segunda opinión clínica o comunicación de riesgo—, que demanda perfiles distintos de sensibilidad y estabilidad.
